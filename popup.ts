// PayPal donation and internationalization for Chrome extension popup with TypeScript
...
interface TimezoneCountryMap {
  [timezone: string]: string;
}

interface LocaleCountryMap {
...
const detectUserCountry = (): string => {
  try {
    ...
    const timezoneCountryMap: TimezoneCountryMap = {
      // Americas
  ...
  try {
    ...
    const localeCountryMap: LocaleCountryMap = {
      "pt-BR": "BR",
      "en-US": "US",
      "en-GB": "GB",
      "en-CA": "CA",
  ...
  try {
    ...
    const language: string = navigator.language || (navigator as any).userLanguage || "en-US";
    if (language.includes("-")) {
      const countryCode: string = language.split("-")[1].toUpperCase();
    ...
    const languageCountryMap: LanguageCountryMap = {
      pt: "BR",
      en: "US",
      fr: "FR",
      de: "DE",
...
const openDonation = (): void => {
  const countryCode: string = detectUserCountry();
  const currencyCode: string = COUNTRY_CURRENCY_MAP[countryCode] || "USD";

  const donationUrl: string = `https://www.paypal.com/donate/?cmd=_donations&business=S34UMJ23659VY&currency_code=${currencyCode}`;
...
document.addEventListener("DOMContentLoaded", function (): void {
  ...
  document.querySelectorAll("[data-i18n]").forEach((element: Element): void => {
    const key: string | null = element.getAttribute("data-i18n");
    if (key) {
      const message: string = chrome.i18n.getMessage(key);
  ...
  document.querySelectorAll("[data-i18n-title]").forEach((element: Element): void => {
    const key: string | null = element.getAttribute("data-i18n-title");
    if (key) {
      const message: string = chrome.i18n.getMessage(key);
  ...
  const donateButton: HTMLElement | null = document.getElementById("donateButton");
  if (donateButton) {
